...
class MainWindow(wx.Frame):
    ...
    def MakeMenuBar(self):
        ...
        # Bind the Cut, Copy and Paste menu selection to their handlers
        for ids in [wx.ID_CUT, wx.ID_COPY, wx.ID_PASTE]:
            self.Bind(wx.EVT_MENU, self.OnText, id=ids)
    ...
    def OnText(self, event):
        ...
        id_2_action = {wx.ID_CUT  : 'Cut',
                       wx.ID_COPY : 'Copy',
                       wx.ID_PASTE: 'Paste'}

        action = id_2_action[event.GetId()]
        # Check if the CanCut, CanCopy, CanPaste return True
        if getattr(self.control, 'Can%s'%action)():
            # Actually call Cut, Copy or Paste
            getattr(self.control, '%s'%action)()
